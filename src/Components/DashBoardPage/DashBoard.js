...
const DashBoard = () => {
  ...
  if (name === "emailId" && /^[\w.%+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(value)) delete newErrors.emailId;
  ...
  setErrors(newErrors);
   ...
   const handleSave = () => {
    ...
    if (!/^[\w.%+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(empData.emailId))
      newErrors.emailId = "Invalid email!";

    if (!/^\d{10}$/.test(empData.phone)) newErrors.phone = "Phone number must be 10!";
    ...
    if (Object.keys(newErrors).length === 0) {
      if (mode === "Add") {       
        const maxId = empDetails.reduce((max, emp) => Math.max(max, parseInt(emp.id)), 0); // find highest emp id       
